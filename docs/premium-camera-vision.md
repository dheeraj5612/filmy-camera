...
| Manual exposure includes shutter and ISO priority; the product site describes manual focus, peaking, and a focus loupe | Filmy's source now has a hardware-backed manual-control foundation. Device acceptance and useful measurement aids such as peaking and a loupe remain premium work. A filter's temperature slider is not sensor white-balance control. |
...
| B — First premium candidate | Monthly StoreKit subscription, full-access introductory trial, free watermarking; full recipe collection and tuning; complete device validation and productization of ISO/shutter/manual focus/capture white balance; histogram, highlight clipping aid, focus peaking/loupe; saved shooting presets | Real controls agree with capture metadata and supported hardware. Purchase, expiry, offline use, restores, and all export paths pass acceptance. Advertise only tools actually delivered. |

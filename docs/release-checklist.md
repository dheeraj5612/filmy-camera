...
- [x] Run a signed Release archive for a generic iOS device destination and validate the app bundle with the Apple Distribution certificate and App Store profile `Filmy Camera App Store 1.0.0`; current archive: `build/FilmyCamera-9af4c07-signed.xcarchive`.
- [x] Export and independently validate the exact-head distribution IPA: `build/export-9af4c07/FilmyCamera.ipa` (SHA-256 `89d1e3cd804d2df12748d0079a7f8de527f405192fdd458c86affb19f9202482`).
...
- Current signed device archive: `build/FilmyCamera-9af4c07-signed.xcarchive` reaches a successful arm64 generic-device archive and contains the app, dSYM, privacy manifest, version `1.0.0`, embedded App Store provisioning profile, and a `FilmyCamera.source-sha` marker matching the exact source revision used for the build. Archive validation passes with Apple Distribution identity `Apple Distribution: DHEERAJ SRINIVASA NAMBURU (6ALSCF5GBV)`. The matching IPA was exported and independently verified; upload remains pending final metadata and authenticated App Store Connect access.

...
- [x] Run a signed Release archive for a generic iOS device destination and validate the app bundle with the Apple Distribution certificate and App Store profile `Filmy Camera App Store 1.0.0`; current archive: `/tmp/filmy-camera-release-current-20260814.xcarchive`.
...
- Current hosted evidence: [run 31773335876](https://github.com/dheeraj5612/filmy-camera/actions/runs/31773335876) passed on the prior source revision; the final source/release-evidence revision has its own hosted run pending; [PR #55](https://github.com/dheeraj5612/filmy-camera/pull/55) remains open and draft.
...
- Current signed device archive: `/tmp/filmy-camera-release-current-20260814.xcarchive` reaches a successful arm64 generic-device archive and contains the app, dSYM, privacy manifest, version `1.0.0`, embedded App Store provisioning profile, and a `FilmyCamera.source-sha` marker matching the exact source revision used for the build. Archive validation passes with Apple Distribution identity `Apple Distribution: DHEERAJ SRINIVASA NAMBURU (6ALSCF5GBV)`.

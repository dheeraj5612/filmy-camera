...
enum ProPhotoEncoder {
    ...
    // Encode the same SDR-to-HDR luminance ratio as a normalized gain-map
    // image. Core Image's implicit HDR-image route is unavailable on some
    // iOS 18 renderers, so supplying the map explicitly keeps the HEIF
    // auxiliary image present after the look has been rendered.
    private static let gainMapKernel = CIColorKernel(source: """
        vec3 linearize(vec3 c) {
            vec3 a = abs(c);
            return sign(c) * mix(a / 12.92, pow((a + 0.055) / 1.055, vec3(2.4)), step(vec3(0.04045), a));
        }
        kernel vec4 filmyGainMap(__sample sdr, __sample hdr, float headroom) {
            vec3 weights = vec3(0.2126, 0.7152, 0.0722);
            float reference = max(dot(linearize(sdr.rgb), weights), 0.001);
            float gain = clamp(dot(linearize(hdr.rgb), weights) / reference, 1.0, headroom);
            float encoded = log2(gain) / max(log2(headroom), 0.0001);
    ...
    static func heifData(image: CIImage, sourceSDR: CIImage, sourceHDR: CIImage?, sourceData: Data,
        ...
        if #available(iOS 18.0, *), options.hdr, let sourceHDR, sourceHDR.contentHeadroom > 1.01,
           ...
           let hdr = kernel.apply(extent: primary.extent, arguments: [primary,
            ...
            let headroom = min(sourceHDR.contentHeadroom, 16)
            let renderedSDR = sized(sourceSDR, resolution: options.resolution)
            let renderedHDR = sized(sourceHDR, resolution: options.resolution)
            ...
            // Supplying the map explicitly is necessary on iOS 18 simulator
            // and is also valid on device. Keep the implicit HDR image as a
            // fallback for renderers that cannot evaluate the map kernel.
